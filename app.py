...
def seed():
    ...
    for nm,cat,pr,de in raw:
        d=nid(db,'dish')
        db['dishes'].append({'id':d,'canteen_id':c,'name':nm,'category':cat,'price':pr,'desc':de,'image':'','avg_rating':0,'review_count':0,'is_available':True,'created_at':datetime.now().isoformat()})
    a=nid(db,'user')
...
def dish_page(id):
    ...
    if request.method=='POST' and user:
        ...
        if rating>=1 and rating<=5:
            review={'id':nid(db,'review'),'user_id':user['id'],'dish_id':id,'rating':rating,'content':content_text,'likes':0,'created_at':datetime.now().isoformat()}
